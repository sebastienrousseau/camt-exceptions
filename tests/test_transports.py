...
from camt_exceptions import _cli, _transports, server
...
def test_version_flag(capsys: pytest.CaptureFixture[str]) -> None:
    with pytest.raises(SystemExit) as info:
        _cli.serve("srv", ["--version"], "camt-exceptions-mcp", "9.9.9")
    assert info.value.code == 0
    assert "camt-exceptions-mcp 9.9.9" in capsys.readouterr().out
