...
# camt-exceptions Governance

This document describes how camt-exceptions is run, how decisions are
made, and how to take on responsibility for it. It exists to make the
project legible and sustainable - and, candidly, to reduce its
dependence on any single person.

## Mission and scope

camt-exceptions generates and validates ISO 20022 Exceptions &
Investigations messages - `camt.056` FI-to-FI Payment Cancellation
Requests and `camt.029` Resolutions of Investigation - and exposes that
as MCP tools / resources / prompts so AI agents and assistants can
raise, structure and check payment-cancellation and investigation flows
against the official schemas. Changes are weighed against that scope:
...
- **Significant changes** (new message types, breaking changes, new
  dependencies, new tools/resources/prompts, a change to the error-dict
  calling convention) need explicit approval from a maintainer in the
  PR, and should start as an issue or discussion. Decisions that shape
  the server are written down in [`docs/adr/`](docs/adr/index.md).
...
Releases follow [`RELEASING.md`](RELEASING.md). camt-exceptions ships on
its own version line, incremented by 0.0.1. Only maintainers publish to
PyPI; release authority rests with the lead maintainer and is expanding
to a second maintainer as a standing goal.
...
1. Contribute a few reviewed PRs in an area
   ...
   the planned message types, examples).
...
camt-exceptions today has **one** maintainer, which is a real risk for a
...
- **The work is legible:** [`ARCHITECTURE.md`](ARCHITECTURE.md) maps the
  ...
  and the tools ship with runnable examples.
